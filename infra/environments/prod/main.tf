...
# 1. Production VPC, Private IP Allocation & Serverless VPC Access Connector
module "vpc" {
  source         = "../../modules/vpc"
  ...
  # Sizing VPC connector for high throughput
  connector_min_instances = 2
  connector_max_instances = 10
  connector_machine_type  = "e2-standard-4"
}

# 2. Artifact Registry for Container Images (Immutable Tags Enforced)
...
# 4. Cloud SQL for PostgreSQL 16 (High Availability, 30-Day Retained PITR, Deletion Protection)
module "cloud_sql" {
...
# 5. Cloud Run v2 Services (API, Web, Worker - Warm Instances & Zero Cold Starts)
module "cloud_run" {
